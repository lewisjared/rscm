...
impl Component for MyComponent {
    ...
    fn solve(&self, t_current: Time, t_next: Time, input_state: &InputState) -> RSCMResult<OutputState> {
        ...
        let emissions = inputs.emissions.at_start();  // Type-safe access
...
- Example:
  ...
  #[derive(Debug, Serialize, Deserialize, ComponentIO)]
  #[inputs(
      erf { name = "Forcing", unit = "W/m^2" },
  )]
  #[outputs(
      regional_uptake { name = "Heat|Regional", unit = "W/m^2", grid = "FourBox" },
  )]
  pub struct MyGridComponent {
      pub efficiency: f64,
  }

  fn solve(&self, t_current: Time, t_next: Time, input_state: &InputState) -> RSCMResult<OutputState> {
      let inputs = MyGridComponentInputs::from_input_state(input_state);
      let erf = inputs.erf.get();  // Use get() for non-state inputs

      // Calculate regional values (FourBoxSlice is automatically generated by the macro)
      let outputs = MyGridComponentOutputs {
          regional_uptake: FourBoxSlice::from_array([
              erf * self.efficiency * 1.2,  // Northern Ocean (higher efficiency)
              erf * self.efficiency * 0.6,  // Northern Land
              erf * self.efficiency * 1.6,  // Southern Ocean
              erf * self.efficiency * 0.6,  // Southern Land
          ]),
      };
      Ok(outputs.into())  // Macro handles StateValue::FourBox wrapping
  }
  ```

**Timestep access methods** (`get()`, `at_start()`, `at_end()`):

**Uniform approach (recommended):**

- **State variables**: Use `at_start()` explicitly - you always want your own previous state
- **All other inputs**: Use `get()` - it handles both exogenous and upstream sources automatically

```rust
// State variables: always at_start()
let prev_temp = inputs.temperature.at_start();

// Non-state inputs: always get()
let erf = inputs.erf.get();
let emissions = inputs.emissions.get();
```

This approach makes components more flexible since they don't know how they'll be wired in a model.
...
|---------------------|--------------|--------|
| State variable (your own previous state) | You, last timestep | `at_start()` |
